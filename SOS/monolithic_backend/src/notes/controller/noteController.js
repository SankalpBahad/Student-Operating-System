...
const createNoteFromPdf = async (req, res) => {
    ...
    try {
        ...
        // Make sure we have a valid uid before attempting to create/update a category
        if (!uid) {
        ...
        try {
            ...
            console.error('Error creating/ensuring category:', catError);
            // Continue with note creation even if category creation fails
            // We'll use 'Uncategorized' as a fallback
        }
...
const summarizeNote = async (req, res) => {
  try {
      ...
      // Better content extraction that handles different content structures
      let noteTextContent = "";
      if (Array.isArray(originalNote.content)) {
          // Loop through each block
          originalNote.content.forEach(block => {
              // Different ways to extract text based on the block structure
              if (block.content && Array.isArray(block.content)) {
                  // Extract text from content array (standard BlockNote format)
                  block.content.forEach(contentItem => {
                      if (contentItem.text) {
                          noteTextContent += contentItem.text + " ";
                      }
                  });
                  noteTextContent += "\n";
              } else if (block.content && typeof block.content === 'string') {
                  // Direct string content
                  noteTextContent += block.content + "\n";
              } else if (block.text) {
                  // Direct text property
                  noteTextContent += block.text + "\n";
              }
          });
      } else if (typeof originalNote.content === 'string') {
          // If content is just a string
          noteTextContent = originalNote.content;
      }

      // If we couldn't extract any content, return error
      if (!noteTextContent.trim()) {
          return res.status(400).json({ message: 'Could not extract content from note to summarize' });
      }
      ...
      try {
          ...
          console.error('Error creating summary category:', catError);
...
const generateQuizQuestions = async (req, res) => {
  try {
      ...
      // If we couldn't extract any content, return error
      if (!noteTextContent.trim()) {
          return res.status(400).json({ message: 'Could not extract content from note to generate quiz' });
      }
      ...
      try {
          ...
          console.error('Error creating quiz category:', catError);
          // Continue anyway, we'll use the name anyway
      }
